...
def plot_pruning_results(random_pruning_results: SweepResult,
    ...
    pd_prune_khighest_ss = _convert_to_series(pd_prune_df[pd_prune_df['data.sample_selector.kwargs.keep_highest'] == True], col_name, 'data.sample_selector.kwargs.fraction')
    pd_prune_klowest_ss = _convert_to_series(pd_prune_df[pd_prune_df['data.sample_selector.kwargs.keep_highest'] == False], col_name, 'data.sample_selector.kwargs.fraction')
    
    # TODO support different sample sizes

    pruning_results = {'rand_prune': rand_prune_ss, 'pd_prune_khighest': pd_prune_khighest_ss, 'pd_prune_klowest': pd_prune_klowest_ss}
